...
# --legend  : Curve title name
#  evaluate target ： loss  bbox_mAP  bbox_mAP_50 bbox_mAP_75
"""
plot_curve ../work_dirs/JujubeSSD/epoch_50.json 
--out  ../work_dirs/JujubeSSD/
--legend bbox_mAP_75
--keys bbox_mAP_75
"""
# --eval_interval 1
...
def plot_curve(log_dicts, args):
    ...
    for i, log_dict in enumerate(log_dicts):
        ...
        for j, metric in enumerate(metrics):
            ...
            ###
            npxs = np.array(xs).reshape(-1,1)
            npys = np.array(ys).reshape(-1,1)
            npdata = numpy.concatenate((npxs,npys),1)
            pathtemp = args.out+ args.legend[0]+'.txt'
            np.savetxt(pathtemp,npdata,fmt='%1.4f')
            ###
            plt.legend()
